...
이 결과는 exploratory pattern으로 해석해야 하며, 학과 추천의 최종 판단으로 사용하지 않는다. silhouette은 IDF 가중에서 오히려 낮아지는데(0.33→0.18), 이는 무변별 대군집을 해체했기 때문이며 silhouette이 조밀한 단일 군집을 보상하는 한계를 보여준다 — 따라서 타당성의 주 기준으로 삼지 않는다. 다음 단계에서는 k-means clustering과 cluster-number sensitivity를 추가하고, 14개 공학-자연 코어 내부 sub-clustering을 정리할 예정이다. 전문가 card sorting 기반 외부 검증은 현재 진행 중이며 Final Report에서 일치도(ARI/NMI)를 보고한다. admission score feasibility와 candidate generation은 future work로 남겨둔다.
